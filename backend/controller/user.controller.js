...
export const getUserProfile = async (req, res) => {
    try {
        ...
        const getProfile = await userModel.findOne({ username: username }).select("-password")
        if (!getProfile) {
...
export const getSuggestedUsers = async (req, res) => {
    try {
        ...
        const filteredUser = users.filter((user) => !usersFollowedByMe.following.includes(user.user._id))
        const suggestedUser = filteredUser.slice(0, 4)
        suggestedUser = suggestedUser.map((user) => user.password = null)
        return res.status(400).json(suggestedUser)
    } catch (error) {
